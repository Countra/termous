...
function FilesWorkspaceContent({
  ...
  const clearDragDropTargetsRef = useRef<() => void>(() => undefined)
  ...
  const handleActiveDirectoryCommitted = useCallback(() => {
    clearDragDropTargetsRef.current()
  }, [])
  ...
  const notifyDroppedPathsUnavailable = () => {
    notification.warning({
  ...
  const {
  ...
  } = useFilesWorkspaceDragController({
    filesTableShellRef,
    activeFileSession,
    activeFileSessionClosing,
    fileActionsEnabled,
    ...
    closingFileSessionIdsRef,
    previewClassNames: {
      root: styles['files-remote-drag-preview'],
      label: styles['files-remote-drag-preview-label'],
      count: styles['files-remote-drag-preview-count'],
    },
    onSelectPaths: setSelectedPaths,
    onSetActiveEntry: setActiveEntry,
    onUploadLocalPaths: (paths, targetPath) => uploadLocalPaths('drop', paths, targetPath),
    onMoveRemotePathsToDirectory: moveRemotePathsToDirectory,
    onRemoteMoveUnavailable: notifyRemoteMoveUnavailable,
    onDroppedPathsUnavailable: notifyDroppedPathsUnavailable,
  })
  clearDragDropTargetsRef.current = clearDropTargets

  const dropTargetDirectory = useMemo(
    () => entries.find(
      (entry) => entry.kind === 'directory' && entry.path === dropTargetDirectoryPath,
    ) ?? null,
    [dropTargetDirectoryPath, entries],
  )
  const remoteMoveTargetDirectory = useMemo(
    () => entries.find(
      (entry) => entry.kind === 'directory' && entry.path === remoteMoveTargetPath,
    ) ?? null,
    [remoteMoveTargetPath, entries],
  )
  const dropTargetDirectoryName = dropTargetDirectory?.name
    ?? (dropTargetDirectoryPath ? remotePathDisplayName(dropTargetDirectoryPath) : '')
  const remoteMoveTargetDirectoryName = remoteMoveTargetDirectory?.name
    ?? (remoteMoveTargetPath ? remotePathDisplayName(remoteMoveTargetPath) : '')
              ...
              <>
                <Table
                  ...
                  onRow={(entry) => ({
                    ...
                    onDragLeave: (event) => leaveRemoteMoveTarget(entry, event),
