...
export interface McpIconProps extends Omit<SVGProps<SVGSVGElement>, 'height' | 'width'> {
  size?: number
}

/** MCP 官方标志的单色版本，颜色跟随当前主题。 */
export function McpIcon({ className, size = 24, ...props }: McpIconProps) {
  return (
    <svg
      {...props}
      className={['mcp-icon', className].filter(Boolean).join(' ')}
      width={size}
      height={size}
      viewBox="7 7 166 166"
      fill="none"
