...
test('独立页和工作站复用同一端口转发实例操作组件', () => {
  ...
  assert.match(actions, /scopedClassName\('forward-runtime-action', 'is-restart'\)/)
  assert.match(actions, /scopedClassName\('forward-runtime-action', 'is-stop'\)/)
})
...
test('运行实例操作组使用稳定尺寸且按钮交互不会产生位移', () => {
  ...
  assert.match(styles, /\.forward-runtime-action:global\(\.ant-btn\)\s*\{[\s\S]*width:\s*28px/)
  assert.match(styles, /\.forward-runtime-action:global\(\.ant-btn\):not\(:disabled\):hover,[\s\S]*transform:\s*none/)
  assert.match(styles, /\.forward-runtime-action\.is-stop:global\(\.ant-btn\):not\(:disabled\):hover/)
})
