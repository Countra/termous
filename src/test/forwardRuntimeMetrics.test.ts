...
test('高频端口转发指标使用无 React 状态的顶层详情浮层', () => {
  ...
  assert.match(source, /const hasDetail = Boolean\(rate \|\| detailValue\)/)
  assert.match(source, /tabIndex=\{hasDetail \? 0/)
  assert.match(source, /detailValue=\{formatForwardDateTime\(forward\.started_at\)\}/)
