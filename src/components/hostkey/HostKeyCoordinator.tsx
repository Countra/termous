...
interface HostKeyCoordinatorProps {
  api: TermousApi
  enabled: boolean
  hosts: Host[]
}
...
export function HostKeyCoordinator({ api, enabled, hosts }: HostKeyCoordinatorProps) {
  ...
  return (
    <Modal
      ...
      keyboard={false}
      mask={{ closable: false }}
      rootClassName="host-key-modal-root"
      className="host-key-modal"
    >
      {challenge ? (
        <section className={`host-key-dialog${changed ? ' is-changed' : ''}`} aria-live="assertive">
          <header className="host-key-dialog-header">
            <span className="host-key-dialog-icon" aria-hidden="true">
...
function AffectedWorkflows({
  ...
  return (
    <div className="host-key-workflows">
      <div className="host-key-workflows-heading">
        <span>{t('hostKey.affectedWorkflows')}</span>
        <strong>{challenge.context_count}</strong>
      </div>
      <div className="host-key-context-list">
