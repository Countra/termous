    ...
    >
      {approval ? (
        <section className={styles.dialog}>
          ...
          ) : approval.kind === 'remoteops' && approval.operation ? (
            <RemoteOpsApprovalOperation operation={approval.operation} />
          ) : (
...
function RemoteOpsApprovalOperation({ operation }: { operation: McpApprovalOperation }) {
  const { t } = useTranslation()
...
function ApprovalValue({ label, value, code = false }: { label: string; value: string; code?: boolean }) {
  return (
