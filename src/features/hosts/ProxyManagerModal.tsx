...
export function ProxyManagerModal({
  ...
  return (
    <Modal
      ...
      mask={{ closable: !busy }}
      title={(
        <span className="proxy-manager-title">
          <Network size={18} aria-hidden="true" />
          <span>{t('proxies.manage')}</span>
          <small>{proxies.length}</small>
        </span>
      )}
      rootClassName="proxy-manager-modal"
      onCancel={() => {
        if (!busy) {
          onClose()
        }
    ...
    >
      <div className="proxy-manager-layout">
        <aside className="proxy-manager-sidebar">
          <div className="proxy-manager-search">
            <Input
          ...
          <div className="proxy-manager-list" role="listbox" aria-label={t('proxies.list')}>
            {filteredProxies.length === 0 ? (
              ...
              <button
                ...
                className={`proxy-manager-row ${editingId === proxy.id ? 'is-active' : ''}`}
                disabled={busy}
                onClick={() => selectProxy(proxy)}
              >
                <span className="proxy-manager-row-icon">
                  <Network size={16} aria-hidden="true" />
                </span>
                <span className="proxy-manager-row-copy">
                ...
                <span className="proxy-manager-row-meta">
        ...
        <section className="proxy-manager-editor">
          <header className="proxy-manager-editor-heading">
            <span>
              {editingProxy
          ...
          <div className="proxy-manager-form">
            <label className="proxy-manager-field">
              ...
              {submitted && errors.name ? (
                <small className="proxy-manager-field-error">
            ...
            <label className="proxy-manager-field">
              ...
              <Select
                ...
                classNames={{ popup: { root: 'termous-select-popup proxy-manager-select-popup' } }}
                disabled={busy}
                options={[
                  {
                    value: 'http_connect',
          ...
          <footer className="proxy-manager-editor-footer">
            {editingProxy ? (
              <Tooltip
